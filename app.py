...
profile = {
    ...
    "current_gpa": (
        prof_row[3]
        if prof_row[3] is not None
        else 3.00
    ),
    "current_credits": (
        prof_row[4]
        if prof_row[4] is not None
        else 60
    )
}
...
def get_live_exam_score(exam_id, database_score):
    ...
    Kullanıcı Not kutusuna yeni bir değer girdiğinde
    henüz Kaydet butonuna basmamış olsa bile Streamlit
    değeri session_state'e koyar.

    Bu fonksiyon önce session_state'e bakar.
    Böylece GPA hesaplaması canlı yapılabilir.
    """
...
def calculate_combined_gpa():
    ...
    if courses_df.empty:
        ...
        return (
            0.0,
            float(profile["current_gpa"]),
            0,
            0
        )


    total_term_credits = int(
        courses_df["credit"].sum()
    )
    ...
    for _, c_row in courses_df.iterrows():
        ...
        for _, exam in exams_df.iterrows():
            ...
            score = get_live_exam_score(
                exam_id,
                exam["score"]
            )


            # 0 notu girilmemiş kabul edilir.
            # Eğer 0 FF olarak değerlendirilmek istenirse
            # bu kontrol kaldırılabilir.
        ...
        if total_weight > 0:
            ...
            # Kredi 0 olan dersler GPA'ya dahil edilmez.
            if c_credit > 0:
...
<strong>Öğrenci / Kullanıcı:</strong>
{html.escape(str(profile_data['name']))}

<br>

<strong>Bölüm / Akademik Düzey:</strong>
{html.escape(str(profile_data['department']))}
-
{html.escape(str(profile_data['grade']))}

<br>

<strong>Girdiği Birikimli AGNO (CGPA):</strong>
{profile_data['current_gpa']:.2f}
/
4.00
...
"""
    ...
    if not df_courses_summary.empty:
        ...
        html_content += df_courses_summary.to_html(
            index=False,
            escape=False
        )
        ...
        html_content += (
            "<p>Kayıtlı ders bulunmamaktadır.</p>"
        )
...
2. Değerlendirmeler & Sınav Not Transkripti
...
"""
    ...
    if not df_exams_summary.empty:
        ...
        html_content += df_exams_summary.to_html(
            index=False,
            escape=False
        )
        ...
        html_content += (
            "<p>Sınav kaydı bulunmamaktadır.</p>"
        )
...
<strong>
[{html.escape(str(n['Ders']))}]
{html.escape(str(n['Konu']))}
</strong>
...
with st.sidebar:
    ...
    if st.button(
        ...
        # Eski canlı not değerlerini temizle
        keys_to_delete = [
...
if (
    ...
    ex_date = datetime.strptime(
        str(df_alert.iloc[0]["event_date"]),
        "%Y-%m-%d"
    ).date()
    ...
    exam_code = html.escape(
        str(df_alert.iloc[0]["code"])
    )

    exam_title = html.escape(
        str(df_alert.iloc[0]["title"])
    )

    exam_date = html.escape(
        str(df_alert.iloc[0]["event_date"])
    )
                ...
                with col_e1:
                    ...
                    if not exams_df.empty:
                        ...
                        for _, ex in exams_df.iterrows():
                            ...
                            # NOT KUTUSU
                            #
                            # key kullanıldığı için kullanıcı değeri
                            # session_state'e otomatik yazılır.
                            # -------------------------------------------------
                            ...
                            with cc:
                                ...
                                if st.button(
                                    ...
                                    # Artık DB'ye kaydedildi.
                                    # session state aynı değeri taşımaya
                                    # devam edebilir.
...
# =============================================================================
    ...
    st.caption(
        "Girdiğiniz vize/quiz notlarına göre "
        "istediğiniz harf notunu alabilmek için "
        "Final sınavından kaç almanız gerektiğini hesaplar."
    )
                ...
                if not exams_df.empty:
                    ...
                    if completed_weight > 0:
                        ...
                        if rem_weight > 0:
                            ...
                            st.write(
                                "Mevcut Tamamlanan Ağırlık: "
                                f"**%{completed_weight:.0f}** "
                                "| "
                                "Kalan Final Ağırlığı: "
                                f"**%{rem_weight:.0f}**"
                            )
...
# =============================================================================
    ...
    st.caption(
        ...
        "ardından dönem derslerinizin harf notlarını "
        "seçerek YENİ GANO'nuzu ve Dönem Ortalamanızı "
        "canlı izleyin."
    )
...
"""
        ...
        st.info(
            ...
            "'⚙️ Ders & Müfredat Yönetimi' sekmesinden "
            "bu dönemin derslerini seçip ekleyin."
        )
...
# =============================================================================
    ...
    if (
        ...
        st.info(
            "Aylık başarı eğrisi için sınav "
            "tarihlerini ve aldığınız notları girmelisiniz."
        )
...
# =============================================================================
    ...
    with st.expander(
        ...
        if not courses.empty:
            ...
            st.info(
                "Önce bir ders eklemeniz gerekiyor."
            )
    ...
    if not df_e.empty:
        ...
        st.info(
            "Henüz sınav veya etkinlik eklenmemiş."
        )
...
# =============================================================================
    ...
    if not courses.empty:
        ...
        if not notes_df.empty:
            ...
            st.info(
                "Bu derse ait henüz not bulunmuyor."
            )
        ...
        st.info(
            "Ders notu eklemek için önce ders ekleyin."
        )
            ...
            for _, r in df_c.iterrows():
                ...
                with col_b:
                    ...
                    if st.button(
                        ...
                        # Canlı session değerlerini temizle
                        keys_to_delete = [
                            k
